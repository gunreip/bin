...
usage(){ cat <<USAGE
$SCRIPT_ID $VERSION
Erzeugt/aktualisiert gemeinsames HTML-Audit: audits/_lx-env/latest.html (Append je Run)
...
# CSS sicherstellen (nur _lx-env.css)
if [ -f "$CSS_FILE" ]; then
...
# Template-CSS aus .css.conf.json (nur audits["_lx-env"])
css_links_head(){
  [ -f "$CFG" ] || { echo "<!-- css.conf.json missing -->"; return 0; }
  command -v jq >/dev/null 2>&1 || { echo "<!-- jq missing -->"; return 0; }
  if ! jq -e '.audits["_lx-env"]' "$CFG" >/dev/null; then
    echo "<!-- css: audits._lx-env missing in config -->"; return 0;
  fi
  mapfile -t RAW < <(jq -r '.audits["_lx-env"].files[]?' "$CFG")
  for raw in "${RAW[@]}"; do
    [ -z "$raw" ] && continue
    expanded="$(printf '%s' "$raw" | envsubst)"
    abs="$(abspath "$expanded")"
    if [ ! -f "$abs" ]; then echo "<!-- missing: $(short "$abs") -->"; continue; fi
...
# Run-Section
build_run_section(){
  local ts_h ts_id; ts_h="$(date '+%Y-%m-%d %H:%M:%S %Z')"; ts_id="$(date +%Y%m%d-%H%M%S)"
  local path_ok bashrc_inc bprof prof; local she cr; local fw_loc="$fw"
  ...
  for file in "${!forensic[@]}"; do
    ...
    echo "      <details><summary>$(short "$file") — $cnt Fundstelle(n)</summary>"
    echo "      <ul>"
...
if [ ! -s "$HTML_FILE" ]; then
  ts_h="$(date '+%Y-%m-%d %H:%M:%S %Z')"
  {
    ...
    printf '  <h2 id="%s-subheader">%s - %s</h2>\n' "$SHARED_ID" "$(readlink -f "$0" 2>/dev/null || printf "%s" "$0")" "$VERSION"
    cat "$RUN_FILE"
    echo '</body></html>'
  } > "$HTML_FILE"
  echo "html:  wrote:      $(short "$HTML_FILE")"
else
  ...
  echo "html:  appended:   $(short "$HTML_FILE")"
fi
rm -f "$RUN_FILE"
