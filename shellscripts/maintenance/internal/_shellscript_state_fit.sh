#!/usr/bin/env bash
# .shellscript_state_fit.sh — markiert ein Zielskript als "fit" (SCRIPT_STATE=1)
# v0.1.0
...
usage() {
  echo "Usage: .shellscript_state_fit --script=<name|path> [--update-checklist]"
  echo "       .shellscript_state_fit <name|path>"
}
...
if grep -q '^SCRIPT_STATE=' "$TARGET"; then
  sed -i '0,/^SCRIPT_STATE=/ s/^SCRIPT_STATE=.*/SCRIPT_STATE=1/' "$TARGET"
else
  if head -n1 "$TARGET" | grep -q '^#!'; then
    awk 'NR==1{print; print "SCRIPT_STATE=1"; next}1' "$TARGET" >"$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"
  else
    printf '%s\n%s\n' "SCRIPT_STATE=1" "$(cat "$TARGET")" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"
  fi
fi

echo "FIT: $(basename "$TARGET")  → SCRIPT_STATE=1"
